...
/**
 * All valid handler types as a constant array.
...
/**
 * HTTP handler types for request authentication/authorization levels.
 *
 * Handler types determine the authentication requirements for a route:
 * - **public**: No authentication required, accessible to all users
 * - **api**: Cookie-based authentication required for API endpoints
 * - **app**: Cookie-based authentication required for application UI endpoints
 *
 * @example
 * const handler = factory.GET({
 *   path: '/api/users',
 *   type: 'api',  // Requires authentication
 *   allowed: ['read:users']
 * }, async (ctx) => { ... });
 */
