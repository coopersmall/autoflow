...
export class SharedHTTPHandler<ID extends Id<string>, T extends Item<ID>> {
   ...
   * @param sharedHttpHandlerCtx - Context with validators, logger, config, service
  ...
  constructor(
    ...
    this.factory = this.dependencies.createHttpRouteFactory({
      appConfig: this.sharedHttpHandlerCtx.appConfig,
      logger: this.sharedHttpHandlerCtx.logger,
    });
  }
