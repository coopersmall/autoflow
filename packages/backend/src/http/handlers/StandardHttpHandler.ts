 ...
 * ```
 */
import type { IHttpRoute, RouteType } from '@backend/http/domain/HttpRoute';
...
import type { StandardService } from '@backend/services/standard/StandardService';
...
export class StandardHTTPHandler<ID extends Id<string>, T extends Item<ID>> {
   ...
   * @param ctx - Context with validators, logger, config, service
  ...
  constructor(
    ...
    this.factory = this.dependencies.createHttpRouteFactory({
      appConfig: this.ctx.appConfig,
