...
import { buildRequestContext } from '@backend/http/handlers/actions/buildRequestContext';
import { extractCorrelationId } from '@backend/http/handlers/actions/extractCorrelationId';
import type { CreateRouteRequest } from '@backend/http/handlers/domain/HttpRouteFactory';
...
import { getMiddlewareForHandler } from './getMiddlewareForHandler';
...
export function createRoute(
  ...
  const fn = async (request: BunRequest): Promise<Response> => {
    const middlewares = actions.getMiddlewareForHandler(ctx, {
      routeType,
      requiredPermissions,
    });
