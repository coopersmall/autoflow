 ...
 *   const userId = context.getParam('id', userIdValidator);
 *   if (userId.isErr()) {
 *     return Response.json({ error: userId.error.message }, { status: 400 });
 *   }
 *
 *   const userService = context.getServiceFactory().getService('users')();
