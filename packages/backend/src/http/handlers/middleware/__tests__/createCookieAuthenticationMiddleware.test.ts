...
describe('createCookieAuthenticationMiddleware', () => {
  ...
  it('should authenticate successfully with valid cookies', async () => {
    ...
    userAuth.authenticate.mockReturnValue(Promise.resolve(ok(mockSession)));
  ...
  it('should return error when authentication fails', async () => {
    ...
    userAuth.authenticate.mockReturnValue(Promise.resolve(err(authError)));
  ...
  it('should pass required permissions to userAuth', async () => {
    const logger = getMockedLogger();
    const userAuth = getMockedUserAuthenticationService();
    const appConfig = getMockedAppConfigurationService();
    ...
    expect(userAuth.authenticate).toHaveBeenCalledWith({
      type: 'jwt',
      token: 'token123',
